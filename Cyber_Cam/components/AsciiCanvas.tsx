...
export const AsciiCanvas: React.FC<AsciiCanvasProps> = ({ options, onCapture }) => {
  ...
  useEffect(() => {
    ...
    const startCamera = async () => {
      try {
        ...
        setError("Unable to access camera. Please allow permission from your browser.");
      }
    };
  ...
  return (
    <div className="relative w-full h-full bg-black">
        ...
        {!error && !isCameraReady && (
            <div className="absolute inset-0 z-30 flex items-center justify-center bg-black/80 px-4 text-center font-mono text-green-500">
        ...
        <video 
            ref={videoRef} 
            className="absolute top-0 left-0 h-px w-px -z-10 opacity-0 pointer-events-none" 
            playsInline 
            autoPlay 
            muted 
        />
        ...
        <div className="absolute bottom-40 left-1/2 z-40 flex -translate-x-1/2 items-center gap-4 sm:bottom-32 sm:gap-8">
            {/* Screenshot Button */}
            <button 
                onClick={handleScreenshotClick}
                className="rounded-full border border-green-500/50 bg-black/60 p-3 text-green-400 transition-all backdrop-blur-md active:scale-95 hover:scale-105 hover:bg-green-900/80 hover:shadow-[0_0_15px_rgba(0,255,0,0.3)] sm:p-4"
                title="Save Snapshot"
            >
            ...
            <button 
                ...
                className="group relative rounded-full border border-green-500/50 bg-green-500/20 p-4 text-green-400 transition-all backdrop-blur-md active:scale-95 hover:bg-green-500/40 hover:shadow-[0_0_25px_rgba(0,255,0,0.5)] sm:p-6"
                title="Scan & Analyze"
            >
                <div className="absolute inset-0 rounded-full border border-green-500 opacity-50 animate-ping"></div>
                <ScanEye className="h-6 w-6 sm:h-8 sm:w-8" />
            </button>
        </div>
    </div>
  );
